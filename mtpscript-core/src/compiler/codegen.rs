...
/// Generate built-in ADT constructors that are always available
fn compile_builtin_adt_constructors() -> String {
    let mut output = String::new();
    output.push_str("// Built-in ADT constructors (Option, Result)\n");
    output.push_str("function Some(v) { return { \"Some\": v }; }\n");
    output.push_str("const None = { \"None\": {} };\n");
    output.push_str("function Ok(v) { return { \"Ok\": v }; }\n");
    output.push_str("function Err(e) { return { \"Err\": e }; }\n\n");
    output
}

/// Generate ADT constructor functions for all variants
fn compile_adt_constructors(ir: &IrProgram) -> String {
    let mut output = String::new();

    // Keep track of already-generated constructors to avoid duplicates
    let mut generated: std::collections::HashSet<String> = std::collections::HashSet::new();
    // Mark built-ins as already generated
    generated.insert("Some".to_string());
    generated.insert("None".to_string());
    generated.insert("Ok".to_string());
    generated.insert("Err".to_string());

    for adt in &ir.adt_types {
        output.push_str(&format!("// ADT constructors for {}\n", adt.name));
        for variant in &adt.variants {
            // Skip if already generated (avoid duplicates for Option/Result)
            if generated.contains(&variant.name) {
                continue;
            }
            generated.insert(variant.name.clone());

            if variant.arity == 0 {
                // Unit variant: const Red = { "Red": {} };
                output.push_str(&format!(
                    "const {} = {{ \"{}\": {{}} }};\n",
                    variant.name, variant.name
                ));
            } else if variant.arity == 1 {
                // Single-argument variant: function Circle(v) { return { "Circle": v }; }
                output.push_str(&format!(
                    "function {}(v) {{ return {{ \"{}\": v }}; }}\n",
                    variant.name, variant.name
                ));
            } else {
                // Multi-argument variant: function Rectangle(a, b) { return { "Rectangle": [a, b] }; }
                let params: Vec<String> = (0..variant.arity).map(|i| format!("a{}", i)).collect();
                output.push_str(&format!(
                    "function {}({}) {{ return {{ \"{}\": [{}] }}; }}\n",
                    variant.name,
                    params.join(", "),
...
pub fn compile_ir_to_js(ir: &IrProgram) -> Result<String, CompileError> {
    ...
    // Generate built-in ADT constructors (Some, None, Ok, Err)
    output.push_str(&compile_builtin_adt_constructors());

    // Generate user-defined ADT constructor functions
    let adt_constructors = compile_adt_constructors(ir);
    if !adt_constructors.is_empty() {
        output.push_str(&adt_constructors);
    }
